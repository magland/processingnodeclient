...
% jfm - fixed the short-circuit operators to be compatible with octave
...
function [hdr, filetype, fileprefix, machine] = load_nii_hdr(fileprefix)
   ...
   if ~isempty(findstr('.nii',fileprefix)) && strcmp(fileprefix(end-3:end), '.nii')
      new_ext = 1;
      fileprefix(end-3:end)='';
   end

   if ~isempty(findstr('.hdr',fileprefix)) && strcmp(fileprefix(end-3:end), '.hdr')
      fileprefix(end-3:end)='';
   end

   if ~isempty(findstr('.img',fileprefix)) && strcmp(fileprefix(end-3:end), '.img')
      fileprefix(end-3:end)='';
   end
