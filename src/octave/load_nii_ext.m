...
function ext = load_nii_ext(filename)
   ...
   if (~isempty(findstr('.nii',filename))) && strcmp(filename(end-3:end), '.nii')
      new_ext = 1;
      filename(end-3:end)='';
   end

   if (~isempty(findstr('.hdr',filename))) && strcmp(filename(end-3:end), '.hdr')
      filename(end-3:end)='';
   end

   if (~isempty(findstr('.img',filename))) && strcmp(filename(end-3:end), '.img')
      filename(end-3:end)='';
   end
